...
const BuildContext = createContext(undefined);
...
export function BuildProvider({ children }) {
  const [currentBuild, setCurrentBuild] = useState(createEmptyBuild());
  const [useUsedPrices, setUseUsedPrices] = useState(false);

  const addComponent = useCallback((category, component) => {
  ...
  const totalPrice = Object.values(currentBuild.components).reduce(
    (sum, component) => {
